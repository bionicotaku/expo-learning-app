...
当前 fullscreen 已经把背景双击接入句子级导航，但实现仍是基于当前播放器时间即时计算目标句，不维护短生命周期导航状态机。后续如果要支持更强的连续跳句体验，再引入“最近导航索引”。
...
- 双击左侧：按 `1000ms` 规则回到当前句或上一句开头
- 双击右侧：跳下一句开头；最后一句内或最后一句后跳视频末尾
- 当前时间在 gap 中时，左双击以前一句 start 为基准套同一 `1000ms` 规则，右双击跳 gap 后下一句
- 字幕不可用时 fallback 到播放器相对 `-5s / +5s`
...
当前实现不维护连续导航上下文；每次背景双击都根据当前播放器时间和 transcript sentences 重新计算目标。

如果后续需要更稳定的连续跳句体验，可以引入短生命周期的“最近导航索引”。它可以解决两个问题：
