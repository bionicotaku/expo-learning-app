...
本文档用于定义当前项目下一步要对齐的 `feed` 读取接口契约。
...
1. 当前仓库代码还没有完成对齐，本文件定义的是下一步要落地的 feed API 设计基线，不等于当前代码已实现状态。
2. 如果其他旧文档仍然把 feed 描述成“带分页参数、带 `nextOffset/hasMore` 的接口”，在 **feed API 契约** 这一点上以后续实现为准，本文件优先。
...
1. 它不是分页接口。
2. 它不接受任何请求参数。
3. 后端固定返回 `8` 条 feed item。
4. 返回体只有一层：`items`。
5. 每个 item 直接携带当前卡片和视频页所需的核心展示字段。
...
- 固定返回 `8` 条 item
- item 字段完整对齐本文档
- `coverImageUrl` / `videoUrl` 使用 8 条真实 clip 资源
- 其余字段使用稳定伪随机派生，保证同一 clip 每次结果一致
- 允许保留人工网络延迟，用于验证 loading 态
...
2. feed query 从分页模型改成单次读取模型
3. `pages/feed` 的卡片映射改成消费真实字段
4. `widgets/media-feature-card` 增加封面图和 fallback
5. `pages/video-detail` 移除对“下一页预取”的依赖

在这些代码改造完成前：

- 当前目录级 `README.md` 仍然可以保留对“当前实现”的说明
- 但新的 feed API 契约以本文档为准
