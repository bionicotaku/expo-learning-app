...
- 可通过 `audio` 在 `subtitle` 右侧显示单词/句子音频播放按钮；两个按钮使用同一个 HLS 视频 URL 和各自 `startMs/endMs` 共用一个临时无头播放器
- 音频加载或播放失败时显示全局 `音频加载失败` error toast
...
- `audio` 由 fullscreen row 组合层用当前 `VideoListItem.videoUrl`、当前 `TranscriptToken.start/end` 和当前 `TranscriptSentence.start/end` 注入；只有字幕入口默认提供
...
- word detail 只控制可选的单词/句子音频临时播放器，不控制 fullscreen 主播放器；fullscreen 字幕入口会在打开 dialog 前申请 playback hold，并通过 `onDismissComplete` 释放
- `audio` 是 shared dialog 内容的可选能力；非字幕入口不传时不显示声音按钮、不创建播放器
- 单词/句子音频按钮共用一个无头 `expo-video` player；播放器不渲染 `VideoView`，由 `expo-video` player 自身随组件卸载释放；卸载时不手动调用 native player 方法
