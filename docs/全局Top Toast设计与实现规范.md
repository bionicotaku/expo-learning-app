...
- 透明度：`0 -> 1`
- 位移：`translateY stack push distance -> 0`
- 缩放：`0.98 -> 1`
- 时长：`220ms`
- easing：`Easing.out(Easing.cubic)`

`stack push distance` 固定按 toast 最小高度、纵向 padding 和 stack gap 推导。  
它必须大于单条 toast 的最小占位距离，用来保证第二条 toast 插入顶部时，新 toast 的进入位移和旧 toast 的下移动画形成一致的“向下推”效果。
...
- 新 toast 进入动画和旧 toast 的 layout 下移动画使用同一时长与 easing，避免移动速度不一致导致动画过程重叠
...
- `topOffset` 必须是正值，当前固定为 `12`，确保 toast 位于灵动岛 / notch 的 safe area 下方
- `zIndex` 高于页面内容
