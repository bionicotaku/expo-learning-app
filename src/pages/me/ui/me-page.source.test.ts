...
describe('me page source', () => {
  it('renders the hardcoded Desk UI without live actions or a week issue bar chart', () => {
    ...
    expect(source).toContain('usePresentChoiceQuestionSetDialog');
    expect(source).toContain('hardcodedChoiceQuestions');
    expect(source).toContain('questions: [item.payload]');
    expect(source).toContain('showProgress: false');
    expect(source).toContain('Multi-question demo');
    expect(source).toContain('Open multi-question choice set');
    expect(source).toContain('questions: hardcodedChoiceQuestions.map((item) => item.payload)');
    expect(source).toContain('showProgress: true');
    expect(source).not.toContain('usePresentChoiceQuestionDialog');
    expect(source).not.toContain('createRandomChoiceQuestionProgress');
    expect(source).not.toContain('questionProgress');
