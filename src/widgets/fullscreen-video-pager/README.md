...
- 在 widget 内部维护局部 HUD：`seek / 2x` 由 `playbackFeedback` 驱动，暂停态由独立 pause indicator 驱动
...
- row-local loading 不再显示文案或整屏暗幕，只保留中心 glass spinner
- 采用三层 overlay 结构：
  - `row-owned content overlay`：每个 row 自己的标题、描述、底部可读性 scrim 和右侧 clear glass action rail
  - `top chrome overlay`：右上 counter
  - `active ephemeral overlay`：当前包含中心 pause indicator、左右 seek HUD、顶部 15% 区域的临时 `2x` HUD
- 首屏定位拆成两条明确路径：
...
- 暂停时显示中心 glass pause indicator，并在约 `3s` 后自动消失；恢复播放时立即消失
- 左右 seek 使用左右分置的 glass icon HUD，仍按短时反馈自动消失
- 左右长按期间持续显示顶部约 `15%` 屏高位置的 `2x` glass pill，松手后立即清掉
